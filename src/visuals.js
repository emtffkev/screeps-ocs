...
const getColourByPercentage = (percentage, reverse) => {
    const value = reverse ? parcentage : 1 - percentage;
...
const storageObject = (vis, store, x, startY) => {
    _(store).map((v, k) => ({r: v, k: v})).forEach(v => vis.text(`${v.r}: ${Util.formatNumber(v.a)}`, x, startY += 0.6, Object.assign({color: getResourceColour(v.r)}, this.toolTipStyle)));
};
...
const Visuals = class {
    ...
    run() {
        ...
        for (const room of Game.rooms) {
            ...
            if (VISUALS.SPAWN) {
                room.structures.spawns.filter(s => s.spawning).forEach(this.drawSpawnInfo);
            ...
            if (VISUALS.SOURCE) {
                room.sources.forEach(this.drawSourceInfo);
            ...
            if (VISUALS.LABS) {
                room.structures.labs.all.forEach(this.drawLabInfo);
                p2.checkCPU('Labs', PROFILING.VISUALS_LIMIT);
            }
            if (VISUALS.CREEP) {
                room.creeps.forEach(this.drawCreepPath);
                p2.checkCPU('Creep Paths', PROFILING.VISUALS_LIMIT);
            }
            if (VISUALS.TOWER) {
                room.structures.towers.forEach(this.drawTowerInfo);
    ...
    drawGlobal() {
        ...
        const BAR_ARGS = [x, BAR_Y, sectionWidth, 1, BAR_STYLE];
        const rect = () => vis.rect(...BAR_ARGS);
        if (VISUALS.ROOM) {
            // GCL
    ...
    drawMineralInfo(mineral) {
        ...
        if (mineral.mineralAmount) {
            vis.text(`Amount: ${formatNum(mineral.mineralAmount)}`, x, y, this.toolTipStyle);
        } else {
            vis.text(`Regen: ${formatNum(mineral.ticksToRegeneration)}`, x, y, this.toolTipStyle);
        }
    }
    ...
    drawStorageInfo(storage) {
        ...
        storageObject.call(this, vis, storage.store, x, y);
    }
    ...
    drawTerminalInfo(terminal) {
        ...
        storageObject.call(this, vis, terminal.store, x, y);
    }
    
    drawTransaction(room) {
    ...
    drawCreepPath(creep) {
        const vis = new RoomVisual(room.name);
        if (creep.action.name === 'idle') return; // don't draw idle path
